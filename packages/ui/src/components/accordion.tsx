...
function AccordionTrigger({
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      ...
      >
        {children}
        <HugeiconsIcon icon={ArrowDown01Icon} strokeWidth={2} data-slot="accordion-trigger-icon" className="pointer-events-none shrink-0 transition-transform duration-200 group-data-[state=open]/accordion-trigger:rotate-180 group-aria-expanded/accordion-trigger:rotate-180" />
      </AccordionPrimitive.Trigger>
...
function AccordionContent({
  ...
  return (
    <AccordionPrimitive.Content
      ...
      className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down data-closed:animate-accordion-up data-open:animate-accordion-down px-2 text-xs/relaxed overflow-hidden"
      {...props}
    >
      <div
        className={cn(
